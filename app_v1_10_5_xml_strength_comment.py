...
# ---------------------------------------------------------------------------------------------------------
# v1.10.5 - XML ingredient strength extraction and comment notification added; listedness display unchanged
...
def extract_strengths_mg(*texts: str) -> Tuple[float, ...]:
    """Extract mg strengths from product name/dosage text.

    Examples handled: "5 mg", "10mg", "2.5 mg / 850 mg", "24mg/26mg & 97mg/103mg".
    ...
    # Decode common XML/HTML escaped ampersand and normalize separators.
...
def extract_ingredient_strengths_mg(drug, ns) -> Tuple[float, ...]:
    """Extract strength from XML ingredient quantity numerator fields.

    Expected E2B/R3-like location:
    kindOfProduct/manufacturedProduct/ingredient/quantity/numerator[@value][@unit]
...
def is_strength_allowed_for_product(product_name: str, observed_strengths) -> bool:
    ...
    Missing strength is not marked non-valid because some XMLs may not carry strength.
    If strength is present, single-ingredient products match any one allowed single strength.
    Combination products require an exact ordered tuple, e.g. (2.5, 850.0).
    """
    allowed = allowed_strengths_for_product(product_name)
    if not allowed:
        return True
    ...
    # Exact match first; important for combination products.
    if observed in allowed:
        return True

    # For single-ingredient products, text may contain more than one marketed strength.
    # Treat as allowed if every extracted single strength exists in the master list.
...
def get_launch_date(product_name: str, strength_mg) -> Optional[date]:
    ...
    # If XML carries a strength and it is outside the approved strength master list,
    # do not return a launch date for that strength.
        ...
        if isinstance(payload, dict) and payload:
            if strength_mg is not None:
                if isinstance(strength_mg, (tuple, list)):
                    # launched_by_strength products are single-ingredient in current launch table;
                    # use the first extracted strength for date lookup.
            ...
            # If strength is missing, fall back to earliest launched strength date.
            ...
            for drug in root.findall('.//hl7:substanceAdministration', ns):
                ...
                if drug_id in suspect_ids:
                    ...
                    if matched_company_prod:
                        ...
                        fallback_strengths = extract_strengths_mg(
                            text_clean,
                            f"{dose_val} {dose_unit}" if (dose_val or dose_unit) else "",
                            form_clean,
                        )

                        # Priority: XML ingredient numerator strength > product-name strength > dosage/dose/formulation text.
                        observed_strengths = ingredient_strengths or product_name_strengths or fallback_strengths
                        ...
                        if observed_strengths and not ingredient_strengths and not product_name_strengths:
                            parts.append(f"Recognized Strength: {format_strength_tuple(observed_strengths)}")
