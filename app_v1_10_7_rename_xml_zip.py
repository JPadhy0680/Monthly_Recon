...
# ---------------------------------------------------------------------------------------------------------
# v1.10.7 - XML renaming ZIP output added; strength only from XML ingredient field/product name
...
def extract_strengths_mg(*texts: str) -> Tuple[float, ...]:
    """Extract mg strengths from product-name text only.
...
def sanitize_filename_component(value: str, fallback: str = "Unknown") -> str:
    """Make a text value safe for use inside a file name."""
...
def build_suggested_xml_filename(safety_id: str, validity_value: str, is_serious: bool, molecule_names: List[str], fallback_stem: str = "XML") -> str:
    """Build XML filename as SafetyID_V/NV_S/NS_MoleculeName.xml."""
    safety_part = sanitize_filename_component(safety_id, fallback_stem)
    validity_part = "NV" if isinstance(validity_value, str) and validity_value.startswith("Non-Valid") else "V"
    seriousness_part = "S" if is_serious else "NS"

    clean_molecules = []
    for molecule in molecule_names:
        mol = sanitize_filename_component(molecule, "")
        if mol:
            clean_molecules.append(mol)
    molecule_part = "+".join(clean_molecules) if clean_molecules else "UnknownMolecule"

    return f"{safety_part}_{validity_part}_{seriousness_part}_{molecule_part}.xml"


def make_unique_filename(filename: str, used_names: Set[str]) -> str:
    """Avoid duplicate file names inside the ZIP by adding _2, _3, etc."""
...
renamed_xml_files: List[Tuple[str, bytes]] = []
used_renamed_xml_names: Set[str] = set()
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            molecule_names_for_filename = [
                ...
                for pnorm in sorted(list(case_products_norm))
            ]
            ...
            suggested_xml_filename = make_unique_filename(suggested_xml_filename, used_renamed_xml_names)
            renamed_xml_files.append((suggested_xml_filename, xml_bytes))
...
with tab2:
    ...
    if all_rows_display:
        ...
        if renamed_xml_files:
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zip_file:
                for renamed_filename, xml_content in renamed_xml_files:
                    zip_file.writestr(renamed_filename, xml_content)
            st.download_button(
                "\u2B07\uFE0F Download Renamed XML Files (.zip)",
                zip_buffer.getvalue(),
                "renamed_xml_files.zip",
